...
it('can add the star name and star symbol properly', async() => {
    ...
    assert.equal(tokenName, 'Pulsar Token');
    assert.equal(tokenSymbol, 'PC');
});
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let starId = 10;
    await instance.createStar('awesome star', starId, {from: user});
    let currentStar = await instance.lookUptokenIdToStarInfo(starId);
    assert.equal(currentStar, 'awesome star');
});